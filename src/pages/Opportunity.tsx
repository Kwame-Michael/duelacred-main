...
import { fetchRecord, createRecord, updateRecord, type AirtableRecord } from "@/lib/supabase";
      ...
      <div className="bg-card rounded-xl border border-border p-8 space-y-6">
        ...
        <div>
          <div className="flex justify-between text-sm mb-2">
            ...
            <span className="font-medium">${(f["Amount Raised"] || 0).toLocaleString()} / ${f["Funding Goal"].toLocaleString()}</span>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { icon: Target, label: "Goal", value: `$${f["Funding Goal"].toLocaleString()}` },
            { icon: TrendingUp, label: "Return", value: `${f["Return Multiple"]}x` },
            { icon: Clock, label: "Duration", value: `${f["Repayment Months"]} mo` },
            { icon: DollarSign, label: "Raised", value: `$${(f["Amount Raised"] || 0).toLocaleString()}` },
          ].map((s) => (
          ...
          <div className="bg-background rounded-lg p-4 space-y-3 border border-border">
            ...
            {amount && parseFloat(amount) >= 10 && (
              <p className="text-sm text-muted-foreground">
                Expected return: <span className="font-semibold text-accent">${(parseFloat(amount) * (f["Return Multiple"] || 1)).toFixed(2)}</span>
              </p>
            )}
            <div className="flex gap-2">
              <Button onClick={handleInvest} disabled={investing} className="flex-1">
